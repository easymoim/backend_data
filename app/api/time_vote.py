...
@router.get("/meeting/{meeting_id}", response_model=List[TimeVoteResponse])
def read_time_votes_by_meeting(meeting_id: UUID, db: Session = Depends(get_db)):
    """모임별 투표 목록 조회"""
    votes = crud.time_vote.get_time_votes_by_meeting(db, meeting_id=meeting_id)
    return votes
